...
public class Menu {
    ...
    private void imprimirOpcoesMenu(String item) {
        for (int i = 0; i < menu.size(); i++) {
            ...
            } else if (item.equals("Bebida") && menu.get(i) instanceof Bebida) {
                System.out.printf("%d) %s\n", i + 1, menu.get(i).getDescricao());

            } else if (item.equals("Sobremesa") && menu.get(i) instanceof Sobremesa) {
                System.out.printf("%d) %s\n", i + 1, menu.get(i).getDescricao());
    ...
    public int getCalorias() {
        int caloriaPrato = menu.get(this.pratoEscolhido).getQtdeCalorias();
        int caloriaBebida = menu.get(this.bebidaEscolhida).getQtdeCalorias();
        int caloriaSobremesa = menu.get(this.sobremesaEscolhida).getQtdeCalorias();
